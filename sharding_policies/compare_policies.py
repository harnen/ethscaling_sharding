...
from pmetis import PMetisPolicy
...
def evaluate_policy(input_file, buffer_ratio, shards_num, shard_capacity, time_interval, policy, inter_cost, chainspace_output=False):
    ...
    with open(input_file, newline='') as csvfile:
        ...
        for row in reader:
            ...
            maxBlockNumber = max(blockNumber, maxBlockNumber)
            ...
            transaction_buffer.append((blockNumber, involved))
            if(len(transaction_buffer) >= buffer_size):
                ...
                rounD += 1
                shard_txs[rounD] = []
                for accounts_set in transaction_buffer: 
                    # A tx is a set of acounts                   
    ...
    return {'steps': steps, 'wasted_capacity': total_wasted, 'inter': total_inter, 'intra': total_intra, 'load': total_load, 'num_migrations' : num_migrations}
...
def main(argv):
    ...
    policy_list.append(MichalPolicy(shards_num, intra_shard_cost=1, inter_shard_cost=inter_cost, global_optimal=True))
    ...
    for policy in policy_list:
        ...
        num_migrations = 0
        if isinstance(policy, PMetisPolicy) or isinstance(policy, DynamicPolicy):
    ...
    keys = ['policy', 'steps', 'wasted_capacity', 'inter', 'intra', 'load', 'inter_cost', 'buffer_size', 'inpurt_file', 'shards_num', 'shard_capacity', 'num_migrations', 'input_file']
