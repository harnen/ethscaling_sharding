...
class MichalPolicy(Policy):
    ...
    def __init__(self, num_shards, intra_shard_cost, inter_shard_cost, global_optimal=False, debug=False):
    ...
    def check_migratibility(self, src, src_shard, dest_shard):
        
        gravity_src = self.G[src][self.shard_nodes[src_shard]]['weight']
        gravity_dest = 0
        if self.G.has_edge(src, self.shard_nodes[dest_shard]):
            gravity_dest = self.G[src][self.shard_nodes[dest_shard]]['weight']

        #assert (self.shard_load[src_shard] >= self.shard_load[dest_shard])

        if (gravity_dest - gravity_src) > (self.shard_load[src_shard] - self.shard_load[dest_shard]):

        #if gravity_dest > 2*gravity_src and self.shard_load[src_shard] > 2*self.shard_load[dest_shard]:
            return True
